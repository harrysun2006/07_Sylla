...
public class OSCacheProvider implements CacheProvider {
	 ...
	 * OSCache {@link Config} which reads the properties file (<code>oscache.properties</code>)
	 * from the classpath. If the file cannot be found or loaded, an the defaults
	 * are used.
	 * 
	 * @param region
	 * @param properties
	...
	public Cache buildCache(String region, Properties properties)
		...
		int refreshPeriod = PropertiesHelper.getInt(StringHelper.qualify(region,
				OSCACHE_REFRESH_PERIOD), OSCACHE_PROPERTIES,
				CacheEntry.INDEFINITE_EXPIRY);
		String cron = OSCACHE_PROPERTIES.getProperty(StringHelper.qualify(region,
				OSCACHE_CRON));
